...
# HyperParameters
with open('configs/SARSA.yml') as f:
...
for episode in range(NUM_EPISODES):
    ...
    while not done:
        ...
        if not done:
            ...
            max_future_q = Q_TABLE[new_discrete_observation + (new_action, )]
            new_q = curr_q + ALPHA*(reward + GAMMA*max_future_q - curr_q)
            Q_TABLE[curr_discrete_observation + (action, )] = new_q
            
            save_scalars('cart_positions', 'SARSA', cart_positions)
            save_scalars('pole_angles', 'SARSA', pole_angles)
    ...
    save_scalars('episode_rewards', 'SARSA', episode_rewards, save_every=1)